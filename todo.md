...
*   [ ] *Corner Case:* Test typing stop sequence slowly vs quickly.
...
*   [ ] *Corner Case:* Test file conflict handling during archive (`gg`).
...
*   [ ] *Refinement:* Specifically test settings UI components for responsiveness and usability on mobile devices.
...
*   [ ] *Performance:* Test `cc` command with very long notes to check parsing performance (non-blocking for MVP, but good to know).
*   [ ] *Validation:* Test `cc`, `gg`, `dd`, `nn` detection logic against the specific incorrect examples provided in `README.md`.
